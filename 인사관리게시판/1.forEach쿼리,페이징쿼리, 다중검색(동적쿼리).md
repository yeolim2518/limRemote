	...
	SELECT 	F.NO,
	   ...
	   FROM (SELECT AA.*,		<!-- ROWNUM :각 row마다 번호를 붙여줘서 우리가 원하는 row들을 가져올수있도록 해줌. 각 테이블마다 자동으로 들어가는 가상컬럼 -->
	           ...
	           FROM (SELECT A.*
	                   FROM EMPLOYEE A
	                  WHERE USE_YN = 'Y'
	                  <if test="searchCol != null and searchCol != '' and !searchCol.equals('TOTAL')" >
	                  	 <!-- <${searchCol}>, LIKE로 문자열 찾고, <"%" || #{searchCol} || "%"> 
	                  	                   이 컬럼(searchCol)에서    뒤에 문자열이 있는지 찾아줌  -->
	                  	 	AND ${searchCol} LIKE '%' || #{searchVal} || '%'  <!-- 이름 %술개발% -->
	                  </if>
	                  <!-- 전체에서 검색할때.. 꼭 AND로 이어주고 ()가로안에 조건 OR로 하기-->
	                  <if test="searchCol != null and searchCol != '' and searchCol.equals('TOTAL')" >
	                  	 	AND (KOR_NAME LIKE '%' || #{searchVal} || '%'  
				       	      OR SEX LIKE '%' || #{searchVal} || '%'  
				       	      OR TECH_LEV LIKE '%' || #{searchVal} || '%')  
	                  </if>
...
<!-- 총 row개수, 총 페이지수 구하는 쿼리     !!!위에 쿼리와 셋트-->
<select id="selectEmpListRowCnt" parameterType="pagingVO" resultType="egovMap">
	SELECT COUNT(*) AS TOTAL_CNT,
	 ...
	 WHERE USE_YN = 'Y' 
      ...
      <if test="searchSex != null and searchSex != '' and !searchCol.equals('TOTAL')">
      		AND SEX = #{searchSex}
      </if>
      <if test="searchTechLev != null and searchTechLev != '' and !searchCol.equals('TOTAL')">
      		AND TECH_LEV = #{searchTechLev}
      </if> 
